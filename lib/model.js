...
Model.prototype.hasAndBelongsToMany = function(joinedModel, fieldDoc, leftKey, rightKey, options) {
  ...
  else if (this._getModel()._name < joinedModel._getModel()._name) {
  ...
  var linkLeftKey = options.linkLeftKey || self.getTableName()+'_'+leftKey
  var linkRightKey = options.linkRightKey || joinedModel.getTableName()+'_'+rightKey
  ...
  this._getModel()._joins[fieldDoc] = {
    ...
    link: link,
    linkModel: linkModel,
    linkLeftKey: linkLeftKey,
    linkRightKey: linkRightKey
  }
  ...
  if (options.init !== false) {
    ...
    if ((this.getTableName() === joinedModel.getTableName())
      && (leftKey === rightKey)) {
      indexName = leftKey+"_"+rightKey
      // The relation is built for the same model, using the same key
      // Create a multi index
      query = r.branch(
        r.table(link).indexList().contains(indexName),
        r.table(link).indexWait(indexName),
        r.table(link).indexCreate(indexName, function(doc) {
          return doc(leftKey+"_"+rightKey)
        }, {multi: true}).do(function() {
          return r.table(link).indexWait(indexName)
        })
      )
    }
    else {
      indexName = linkLeftKey
      query = r.branch(
        r.table(link).indexList().contains(indexName),
        r.table(link).indexWait(indexName),
        r.table(link).indexCreate(indexName).do(function() {
          return r.table(link).indexWait(indexName)
        })
      ).do(function() {
        indexName = linkRightKey
        return r.branch(
          r.table(link).indexList().contains(indexName),
          r.table(link).indexWait(indexName),
          r.table(link).indexCreate(indexName).do(function() {
